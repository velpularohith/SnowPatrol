...
def create_license_usage_prediction_udf(session: Session, udf_name: str, model, predictor_cols, stage_loc, py_packages, custom_packages=[], probability_no_login_revocation_threshold=0.5):
    input_df_type = T.PandasDataFrameType([T.FloatType() for _ in predictor_cols])
    return_df_type = T.PandasSeriesType(T.FloatType())

    @F.udf(name=udf_name, is_permanent=True, stage_location=stage_loc, max_batch_size=1000, packages=py_packages, imports=custom_packages, replace=True, session=session
...
def binarize_and_stuff_data(auth_logs, work_days, employee_metadata, whitelisted_users, cutoff_days, target_variable_no_login):
    ...
    employee_metadata_1 = employee_metadata.filter(~F.call_udf(f"{SNOWPATROL_DB}.{SNOWPATROL_SCHEMA}.udf_contains_anyof", F.col("title"), F.lit(TITLES_KW_WITH_ACCESS_RETAINED)))
    ...
    offset_days = cutoff_days
    ex_emp_cutoff_date = today - timedelta(days = offset_days)

    # window1 = Window.partition_by("session_user")

    filtered_logs_3 = filtered_logs_2.join(employee_metadata_1, 
                                             on=(F.lower(filtered_logs_2.session_user) == F.lower(employee_metadata_1.session_user)), how="inner", lsuffix="_l", rsuffix="_r")\
    ...
    logins_per_user_per_day = filtered_logs_3.select(F.col("session_user"), F.col("authentication_datetime"), F.to_date(F.col("authentication_datetime")).alias("authentication_date"), F.col("title"),  F.col("division"),  F.col("department"))\
    ...
    all_workdays_with_session_user_1 = work_days_1.cross_join(filtered_logs_3)\
        .group_by(F.col("snapshot_datetime"), F.col("session_user")).agg(F.col("snapshot_datetime"), F.col("session_user"), F.max("work_day").alias("work_day"))
    
    all_workdays_with_session_user_2 = all_workdays_with_session_user_1.join(logins_per_user_per_day, how="leftouter"
                                        ,on=((all_workdays_with_session_user_1['snapshot_datetime'] == logins_per_user_per_day['authentication_date']) 
                                                & 
                                                (all_workdays_with_session_user_1['session_user'] == logins_per_user_per_day['session_user'])
                                            )
                                        ,lsuffix="_l"
                                        ,rsuffix="_r")\
                                    .with_column_renamed("session_user_l", "session_user")\
    ...
    cutoff_date = F.lit(max_date)
    filtered_data = all_workdays_with_session_user_2.filter(F.col("snapshot_datetime") <= cutoff_date) 
...
def generate_feature_data_set(app_id:int
   ...
   bin_data = binarize_and_stuff_data(auth_logs, work_days, employee_metadata, whitelisted_users, cutoff_days=cutoff_days, target_variable_no_login = target_variable_days_no_login)
   ...
   logins_before_cutoff_2 = logins_before_cutoff_1.filter(F.col("first_authentication") < F.lit(cutoff_date))
   ...
   logins_before_cutoff_3 = logins_before_cutoff_2.group_by(F.col("session_user"))\
                              ...
                              .with_column("training_date", F.lit(datetime.now().date()))\
   ...
   logins_after_cutoff_2 = logins_after_cutoff_1.group_by(F.col("session_user"))\
    .agg([
        F.max(F.col("days_until_next_login")).alias("days_until_next_login")
    ])\
    .with_column("did_not_login", F.iff(F.col("days_until_next_login") <= F.lit(target_variable_days_no_login), 1, 0))
...
def run_model_today(session: Session
                                   ,app_id:int
                                   ,cutoff_days: int
    ...
    okta_logs = session.table(f"{SNOWPATROL_DB}.{SNOWPATROL_SCHEMA}.{TBL_OKTA_USERS}")\
        .select( 
    ...
    app_logs = session.table(f"{SNOWPATROL_DB}.{SNOWPATROL_SCHEMA}.{TBL_APP_LOGS}")\
        .select(
            F.col('"SESSION_USER"').alias('session_user')
            , F.to_date(F.col('"SNAPSHOT_DATETIME"')).alias('authentication_datetime')
            , F.col('"APP_ID"').alias('app_id')
    ...
    work_days = session.table(f"{SNOWPATROL_DB}.{SNOWPATROL_SCHEMA}.{TBL_WORK_DAYS}")\
    .select(
        F.iff(F.col('"WORK_DAY"'),1,0).alias('work_day')
        , F.col('"SNAPSHOT_DATETIME"').alias('snapshot_datetime')
        )

    employee_metadata = session.table(f"{SNOWPATROL_DB}.{SNOWPATROL_SCHEMA}.{TBL_EMPLOYEE_METADATA}")\
    ...
    # today = datetime.now().date()
    # max_date = today - timedelta(days=cutoff_days)
    # sql_query = f"SELECT e.session_user, a.snapshot_datetime, DATEDIFF(DAY, a.snapshot_datetime,'{today}') AS days_between FROM snowpatrol_.main.sample_employee_metadata e JOIN snowpatrol_.main.sample_app_logs a ON e.session_user = a.session_user WHERE a.snapshot_datetime <= '{today}'"
    # df = session.sql(sql_query).to_pandas()

    # Convert the result into a Pandas DataFrame

    whitelisted_users = session.table(f"{SNOWPATROL_DB}.{SNOWPATROL_SCHEMA}.{TBL_WHITELISTED_USERS}").select(F.col('"EMAIL"').alias("email"))
    ...
    train_user_kpis = generate_feature_data_set(
                                ...
                                ,employee_metadata
                                ,whitelisted_users
                                ,cutoff_days = cutoff_days # Cutoff days allows us to create more folds into the past, counting back from current-date
    ...
    # today = datetime.now().date()
    # cut_date = today - timedelta(days = cutoff_days)
    # train_user_kpis=train_user_kpis.assign(cutoff_date=cut_date)
    # train_user_kpis=train_user_kpis.assign(fold=cutoff_days)
    train_user_kpis.columns = train_user_kpis.columns.str.upper()
    train_user_kpis = train_user_kpis.assign(run_id = this_run_id)
    session.write_pandas(train_user_kpis, f"{TBL_LICENSE_USAGE_FEATURES}",database=f"{SNOWPATROL_DB}",schema=f"{SNOWPATROL_SCHEMA}" ,auto_create_table=True)
    ...
    try:
        ...
        #     # Inside run_model_today function
        #     train_predictor_variables = train_user_kpis.loc[:, predictor_columns].reset_index(drop=True)
        #     target_variables = train_user_kpis['did_not_login']  # Use the binary target variable
        ...
        #             , pd.get_dummies(
        #                 train_user_kpis.assign(department = np.where(pd.isna(train_user_kpis.DEPARTMENT), 'none', train_user_kpis.DEPARTMENT.str.lower().str.replace(' ', '_')))['department']).reset_index(drop = True)
        #         ], axis = 1)

        #         test_predictor_variables = pd.concat([
        #             test_predictor_variables
        #             , pd.get_dummies(
        #                 test_user_kpis.assign(department = np.where(pd.isna(test_user_kpis.DEPARTMENT), 'none', test_user_kpis.DEPARTMENT.str.lower().str.replace(' ', '_')))['department']).reset_index(drop = True)
        #         ], axis = 1)
        ...
        #             train_predictor_variables
        #             , pd.get_dummies(
        #                 train_user_kpis.assign(division = np.where(pd.isna(train_user_kpis.DIVISION), 'none', train_user_kpis.DIVISION.str.lower().str.replace(' ', '_')))['division']).reset_index(drop = True)
        #         ], axis = 1)

        #         test_predictor_variables = pd.concat([
        #             test_predictor_variables
        #             , pd.get_dummies(
        #                 test_user_kpis.assign(division = np.where(pd.isna(test_user_kpis.DIVISION), 'none', test_user_kpis.DIVISION.str.lower().str.replace(' ', '_')))['division']).reset_index(drop = True)
        #         ], axis = 1)
        ...
        #             train_predictor_variables
        #             , pd.get_dummies(
        #                 train_user_kpis.assign(title = np.where(pd.isna(train_user_kpis.TITLE), 'none', train_user_kpis.TITLE.str.lower().str.replace(' ', '_')))['title']).reset_index(drop = True)
        #         ], axis = 1)

        #         test_predictor_variables = pd.concat([
        #             test_predictor_variables
        #             , pd.get_dummies(
        #                 test_user_kpis.assign(title = np.where(pd.isna(test_user_kpis.TITLE), 'none', test_user_kpis.TITLE.str.lower().str.replace(' ', '_')))['title']).reset_index(drop = True)
        #         ], axis = 1)
        ...
        #     session.file.put(model_file, f"@{SNOWPATROL_DB}.{SNOWPATROL_SCHEMA}.{MODELS_STAGE}",overwrite=True, auto_compress=False)
        #     exec_status = {
        #     "status": f"INFO: Model trained as @{SNOWPATROL_DB}.{SNOWPATROL_SCHEMA}.{MODELS_STAGE}/{model_name}",
        #     "run_id": None,
        #     "target_table": None
        #     }
        ...
        #         session
        #         ,udf_name=f"{SNOWPATROL_DB}.{SNOWPATROL_SCHEMA}.udf_predict_login_probability"
        #         ,model=trained_logistic_regression_model
        #         ,predictor_cols=predictor_columns
        #         ,stage_loc=f"@{SNOWPATROL_DB}.{SNOWPATROL_SCHEMA}.{OBJECT_STAGE}"
        #         ,py_packages=['snowflake-snowpark-python','pandas','numpy', 'scikit-learn==1.2.1']
        #     )
        # today = datetime.now().date()
        # cut_date = today - timedelta(days = cutoff_days)
        # test_user_kpis = test_user_kpis.assign(cutoff_date=cut_date)
        # test_user_kpis = test_user_kpis.assign(fold=cutoff_days)
        ...
        test_user_kpis = test_user_kpis.assign(revoke=1)
        ...
        session.write_pandas(test_user_kpis, f"{TBL_LICENSE_REVOCATION_RECOMMENDATION}", database=f"{SNOWPATROL_DB}",schema=f"{SNOWPATROL_SCHEMA}" ,auto_create_table=True, overwrite=False)
        
        exec_status = {
            "status": "SUCCESS",
            "run_id": this_run_id,
            "target_table": f"{SNOWPATROL_DB}.{SNOWPATROL_SCHEMA}.{TBL_LICENSE_REVOCATION_RECOMMENDATION}"
        }
            # license_revocation_recommendation = session.create_dataframe(test_user_kpis)
            # license_revocation_recommendation.write.mode("overwrite").save_as_table(f"{SNOWPATROL_DB}.{SNOWPATROL_SCHEMA}.license_revocation_recommendation")
